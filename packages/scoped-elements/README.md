...
### What if I need to use the component tag?

Sometimes we need to use the component tag name, for example, if we want to do a
`querySelector`. In that case there is a second element returned by
`createScopedHtml` function which is a map with the tags created for each
component.
...
const { html, tags } = createScopedHtml({
  'my-button': MyButton,
  'my-panel': MyPanel,
});
```

So now we could do the querySelector using those tag names.

```js
const myButton = document.querySelector(`div ${tags['my-button']}`);
```
